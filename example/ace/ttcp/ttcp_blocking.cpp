...
void transmit(const Options& opt)
{
    ...
    if (write_n(sockfd, &session_message, sizeof(session_message)) != sizeof(session_message))
    {
        perror("write session_message");
        exit(1);
    }
...
void receive(const Options& opt)
{
    ...
    SessionMessage session_message = {0, 0};
    if (read_n(sockfd, &session_message, sizeof(session_message)) != sizeof(session_message))
    {
        perror("read session_message");
        exit(1);
    }
    ...
    const int total_len = static_cast<int>(sizeof(int32_t) + session_message.length);
    PayloadMessage* payload = static_cast<PayloadMessage*>(::malloc(total_len));
    assert(payload);

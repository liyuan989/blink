...
void test(const TimeZone& tz, TestCase testcase)
{
	time_t gmt = getGmt(testcase.gmt);
	{
	struct tm local = tz.toLocalTime(gmt);
	char buf[256];
	strftime(buf, sizeof(buf), "%F %T%z(%Z)", &local);
	if ((strcmp(buf, testcase.local) != 0) || (testcase.is_dst != local.tm_isdst))
	{
		//printf("buf = %s  testcase.local = %s\n", buf, testcase.local);
		//printf("testcase.is_dst = %d  local.tm_isdst = %d\n", (int)testcase.is_dst, (int)local.tm_isdst);
		printf("error: \n");
	}
	printf("%s -> %s\n", testcase.gmt, buf);
	}
	...
	{
	struct tm local = getTm(testcase.local);
...
void testUtc()
{
	...
	for (time_t i = -kRange; i <= kRange; i += 11)
	{
		struct tm* t1 = gmtime(&i);
		struct tm t2 = TimeZone::toUtcTime(i, true);
		char buf1[80] = {0};
		char buf2[80] = {0};

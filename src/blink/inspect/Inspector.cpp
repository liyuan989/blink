...
#include <blink/inspect/SystemInspector.h>
...
Inspector::Inspector(EventLoop* loop,
                     const InetAddress& http_addr,
                     const string& name)
    : server_(loop, http_addr, name),
      ...
      system_inspector_(new SystemInspector)
{
    ...
    system_inspector_->registerCommands(this);
#ifdef HAVE_TCMALLOC
    performance_inspector_.reset(new PerformanceInspector);
    performance_inspector_->registerCommands(this);
#endif
...
void Inspector::onRequest(const HttpRequest& request, HttpResponse* response)
{
    if (request.path() == "/")
    {
        string result;
        MutexLockGuard guard(mutex_);
        for (std::map<string, HelpList>::const_iterator helps_iter = helps_.begin();
             helps_iter != helps_.end(); ++helps_iter)
        {
            const HelpList& list = helps_iter->second;
            for (HelpList::const_iterator it = list.begin();
                 it != list.end(); ++it)
            {
                result += "/";
                result += helps_iter->first;
                result += "/";
                result += it->first;
                size_t len = helps_iter->first.size() + it->first.size();
        ...
        response->setContextType("test/plain");
        response->setBody(result);
    }
    else
    {
        std::vector<string> result = split(request.path());
        //boost::split(result, request.path(), boost::is_any_of("/"));
        ...
        else if (result.size() == 1)
        {
            ...
            {
                LOG_DEBUG << "Unimplemented " << module;
            }
        }
        else
        {
            string module = result[0];
            std::map<string, CommandList>::const_iterator modules_iter = modules_.find(module);
            if (modules_iter != modules_.end())
            {
                string command = result[1];
                const CommandList& command_list = modules_iter->second;
                CommandList::const_iterator it = command_list.find(command);
